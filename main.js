...
function checkCoinConfig(callback) {
    ...
    // generic locations for cbtc, zcash, and zencash
    let cbtcPath, zecPath, zenPath;
    ...
    if ((config.coin.toLowerCase() === "cbtc") && (!fs.existsSync(app.getPath("appData") + cbtcPath + "/cbtc.conf"))) {
        if (!fs.existsSync(app.getPath("appData") + cbtcPath)) {
            fs.mkdirSync(app.getPath("appData") + cbtcPath);
        }
        let data = [
            "rpcuser=cbtcrpc",
        ...
        fs.writeFileSync(app.getPath("appData") + cbtcPath + "/cbtc.conf", data.join("\n"));
    }
...
function startWallet() {
    ...
    // if we are configured for cbtc then do cbtc stuff bro
    if (config.coin.toLowerCase() === "cbtc" || config.coin.toLowerCase() === "") {
        if (os.platform() === "win32") {
            cmd = config.binaryPathWin.length > 0 ? config.binaryPathWin : (app.getAppPath() + "/cbtcd.exe");
        }
        else if (os.platform() === "darwin") {
            cmd = config.binaryPathMacOS.length > 0 ? config.binaryPathMacOS : (app.getAppPath() + "/cbtcd-mac");
        }
        else if (os.platform() === "linux") {
            cmd = config.binaryPathLinux.length > 0 ? config.binaryPathLinux : (app.getAppPath() + "/cbtcd-linux");
        }
    }
    // if not then try zec
            ...
            submenu: [
                {
                    ...
                    click() {
                        dialog.showOpenDialog(getFileLocationOpts("Import eleos-wallets.tar"), function (path) {
                            ...
                            // get classicbitcoin path and backup current wallet
                            let cbtcPath;
                            if ((os.platform() === "win32") || (os.platform() === "darwin")) {
                                cbtcPath = app.getPath("appData") + "/" + "cbtc/wallet.dat";
                            }
                            if (os.platform() === "linux") {
                                cbtcPath = app.getPath("home") + "/" + "./cbtc/wallet.dat";
                            }
                            if (fs.existsSync(cbtcPath)) {
                                let newPath = cbtcPath + ".bak-" + new Date().getTime();
                                fs.createReadStream(cbtcPath).pipe(fs.createWriteStream(newPath));
                            }
                            ...
                            tarball.pipe(tar.extract(extractDir, {
                                map: function (header) {
                                    if (header.name.toLowerCase().search("cbtc") !== -1) {
                                        header.name = cbtcPath;
                ...
                {type: "separator"},
                {
                    label: "Restore ZEN from CBTC wallet.dat",
                    click() {
                    ...
                    submenu: [
                        {
                            label: "cbtc (CBTC)",
                            type: "radio",
                            checked: (config.coin.toLowerCase() === "cbtc"),
                            click() {
                                config.coin = "cbtc";
                                writeConfig(JSON.stringify(config, null, 4));
