...
if ((config.confPathWin.length > 0 && fs.existsSync(config.confPathWin)) ||
    ...
    if ((config.coin.toLowerCase() === "cbtc") && ((os.platform() === "win32") || (os.platform() === "darwin"))) {
        coinConf = app.getPath("appData") + "/cbtc/cbtc.conf";
    } else if (config.coin.toLowerCase() === "cbtc") {
        coinConf = app.getPath("home") + "/.cbtc/cbtc.conf";
    }
...
rl.on("close", function () {
    ...
    //rpcPort = rpcOpts.rpcport ? rpcOpts.rpcport : (config.rpcPort.length > 0 ? config.rpcPort : (config.coin == "cbtc" ? 8232 : 8233));
    if (rpcOpts.rpcport) {
...
ipcMain.on("coin-request", function (event) {
    if (config.coin.length === 0) {
        event.sender.send("coin-reply", "cbtc");
    } else {
